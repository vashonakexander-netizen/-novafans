...
# Automated scraper runner for toflx.com
# This script will start the API server and run the scraper
...
echo "🎬 Starting Scraper for toflx.com"
echo "═══════════════════════════════════════════════"
echo ""
...
if curl -s "$API_URL/health" > /dev/null 2>&1; then
  ...
  pnpm dev > /tmp/api-scraper.log 2>&1 &
  API_PID=$!
  echo "   API server PID: $API_PID"
  echo "   Logs: tail -f /tmp/api-scraper.log"
  
  echo ""
  echo "⏳ Waiting for API server to start (up to 30 seconds)..."
  for i in {1..15}; do
    if curl -s "$API_URL/health" > /dev/null 2>&1; then
  ...
  if ! curl -s "$API_URL/health" > /dev/null 2>&1; then
    ...
    echo "   Check logs: tail -f /tmp/api-scraper.log"
    exit 1
  fi
fi
